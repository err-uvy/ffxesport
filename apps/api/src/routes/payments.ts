...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const raw = getRawBody(
      req.body,
      (req as RawBodyRequest).rawBody ?? Buffer.from("")
    );
    ...
    await prisma.paymentLog.create({
      data: {
        ...
        eventId:
          String(req.headers["x-razorpay-event-id"] || "") || undefined,
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    await prisma.paymentLog.create({
      data: {
        ...
        eventId: payload.event_time
          ? `${orderId}-${payload.event_time}`
          : undefined,
    ...
    if (!verified) {
      throw new ApiError(400, "Invalid Cashfree signature");
    }
