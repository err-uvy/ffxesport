...
const utils_1 = require("@ffx/utils");
...
async function main() {
    const permissionRows = await Promise.all(permissions.map((key) => {
    ...
    const admin = await prisma.user.upsert({
        ...
        create: {
            ...
            passwordHash: await (0, auth_1.hashPassword)("Admin@12345"),
            status: "ACTIVE",
            emailVerified: true,
    ...
    const player = await prisma.user.upsert({
        ...
        create: {
            ...
            passwordHash: await (0, auth_1.hashPassword)("Player@12345"),
            status: "ACTIVE",
            emailVerified: true,
            referralCode: "NEON25",
    ...
    for (const item of tournaments) {
        ...
        const slug = (0, utils_1.slugify)(item.title);
        ...
        await prisma.tournament.create({
            data: {
                title: item.title,
                slug,
                code: (0, utils_1.buildTournamentCode)(),
                game: item.game,
                mode: item.mode,
                ...
                prizeDistributions: {
                    create: (0, utils_1.calculatePrizeDistribution)(item.prizePool, 3).map((entry) => ({
                        rank: entry.rank,
                        amount: entry.amount
                    }))
                },
