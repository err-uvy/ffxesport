...
import { BarChart3, ClipboardCheck, LifeBuoy, LogOut, Menu, ScrollText, Shield, Trophy, Users, Wallet, X } from "lucide-react";
import { Button, cn } from "@ffx/ui";
import { api } from "@/lib/api";

const adminRoles = ["SUPER_ADMIN", "ADMIN", "MODERATOR", "SUPPORT"];
const userApp = process.env.NEXT_PUBLIC_USER_APP_URL ?? "http://localhost:3000";

const nav = [
  { href: "/admin", label: "Overview", icon: BarChart3 },
  { href: "/admin/users", label: "Users", icon: Users },
  { href: "/admin/tournaments", label: "Tournaments", icon: Trophy },
  { href: "/admin/matches", label: "Matches", icon: ClipboardCheck },
  ...
  useEffect(() => {
    api
      ...
      .then((response) => {
        ...
        if (!current.roles?.some((role: string) => adminRoles.includes(role))) {
          window.location.href = `${userApp}/dashboard`;
          return;
        }
        setUser(current);
        setChecking(false);
