...
import { BadgeCheck, Camera, Crosshair, Trophy } from "lucide-react";
...
type Profile = {
  user: {
    ...
    avatarUrl?: string;
    bio?: string;
    gameProfiles: { id: string; game: string; uid: string; handle?: string; kdRatio: string; verified: boolean }[];
  };
...
export default function ProfilePage() {
  ...
  async function uploadAvatar(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      await api.post("/profile/avatar", body, { headers: { "content-type": "multipart/form-data" } });
  ...
  async function addGameProfile() {
    try {
      await api.post("/profile/game-profiles", { game, uid, handle });
      ...
      <div className="grid gap-5 xl:grid-cols-[.8fr_1.2fr]">
        <Card className="p-5">
          <div className="flex items-center gap-4">
            <div className="h-20 w-20 rounded-xl bg-cover bg-center shadow-neon" style={{ backgroundImage: `url(${profile?.user.avatarUrl ?? "https://images.unsplash.com/photo-1542751371-adc38448a05e?auto=format&fit=crop&w=300&q=80"})` }} />
            <div>
          ...
          <div className="mt-5 grid grid-cols-3 gap-3">
            <Stat icon={Trophy} value={String(profile?.stats.wins ?? 0)} label="Wins" />
            <Stat icon={Crosshair} value={String(profile?.stats.kills ?? 0)} label="Kills" />
            <Stat icon={BadgeCheck} value={profile?.stats.kdRatio ?? "0.00"} label="KD" />
          </div>
          <Textarea className="mt-5" value={bio} onChange={(event) => setBio(event.target.value)} placeholder="Bio" />
          <Button className="mt-3 w-full" onClick={saveBio}>
            Save profile
          </Button>
        </Card>
        <Card className="p-5">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-3">
            ...
            <Input placeholder="Gaming UID" value={uid} onChange={(event) => setUid(event.target.value)} />
            <Input placeholder="Handle" value={handle} onChange={(event) => setHandle(event.target.value)} />
          </div>
          <Button className="mt-3" onClick={addGameProfile}>
            Link UID
          </Button>
